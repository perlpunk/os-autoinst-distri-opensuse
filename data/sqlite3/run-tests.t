...
subtest create_tables => sub {
    ...
    my @output = sqlite_select("SELECT * FROM movies ORDER BY mid");
...
subtest test_alter_tables => sub {
    ...
    my @output   = sqlite_select("SELECT * FROM movie ORDER BY mid LIMIT 1");
    my @expected = (
        "mid|name|year|mtime",
        "1|The Dead Dont Die|2019|",
    );
...
sub sqlite_select {
    my ($select) = @_;
    chomp(my @output = qx{echo "$select" | $sqlite_select });
    return @output;
}
...
sub call_sqlite_file {
    ...
    if ($? != 0) {
        fail $label;
        BAILOUT("Command '$sqlite_select' exited with error, aborting");
        exit 1;
    }
    else {
        pass $label;
